...
output$orrg_presencia_emd <- renderDataTable({
    ...
    if (input$seleccionar_tipo_diocesis_emd == "Todos") {
        mostrar_previo <- orrg_presencia_emd
    }
    else if (input$seleccionar_tipo_diocesis_emd == "Bizancio") {
        mostrar_previo <- orrg_presencia_emd %>%
            filter(country %in% paises_bizancio)
    }
    else if (input$seleccionar_tipo_diocesis_emd == "Extraeuropeas") {
        mostrar_previo <- orrg_presencia_emd %>%
            filter(country %in% paises_noeuropa)
    }
    
    mostrar <- mostrar_previo %>%
        filter(religious_order == input$seleccionar_orrg_presencia_emd) %>%
...
output$orrg_presencia_edm <- renderDataTable({
    mostrar <- orrg_presencia_edm %>%
        filter(religious_order == input$seleccionar_orrg_presencia_edm) %>%
        select(religious_order, diocese_name, country, total) %>%
        arrange(-total)
